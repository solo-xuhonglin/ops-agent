...
| Agent | Python + LangGraph（决策图）+ ChatOpenAI→DeepSeek（function calling） |
...
✅ **已闭环**：数据 → 训练 → 模型版本 → serving 部署 → 代理预测；**Agent 全链路**：gRPC 长连接（M1）→ LangGraph 决策 + 动态工具 schema（M2）→ 处置建议 + grantKey 授权闭环（M3）→ 前端助手浮窗（M4）。
