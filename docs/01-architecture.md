...
Agent 作为 **gRPC client 出站拨号** admin 的 gRPC server（内网 `:9090`，不映射宿主），自身**零监听端口、物理上不暴露任何接口**。整条链路只有一条双向流，承载全部交互：
...
> **断线重连**：指数退避（1s → 30s + 抖动）；重连成功后自动重新 `Register`，无需人工介入。admin 侧 `WorkerRegistry` 以心跳 90s 超时清理失联 Agent。
