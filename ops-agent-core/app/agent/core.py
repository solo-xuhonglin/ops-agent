...
SYSTEM_PROMPT = (
    ...
    "    1) 不要调用写工具（即使你能）；\n"
    "    2) **必须在最终回答 content 末尾**追加 ```json {\"suggestions\":[{\"action_type\":\"...\","
    "\"target_type\":\"...\",\"target_id\":N,\"params\":{...},\"reason\":\"...\","
    "\"priority\":\"HIGH|NORMAL|LOW\"}]} ``` 代码块说明需要的写操作（注意：必须出现在最终回答 content，"
    "不能只出现在 reasoning；reasoning 不入库，用户看不到也无法触发审批）；\n"
