"""LangGraph 决策图专项测试：多轮工具循环、recursion 上限恢复。"""
import pytest

from app.agent.context import TaskContext
from app.agent.graph import build_graph, run_graph
from app.tools.registry import ToolRegistry
from tests.test_agent_core import FakeClient, FakeHttp, FakeLlm, make_tool, tool_call_msg
    ...
    async def chat(self, messages, tools=None):
        if messages[-1].get("role") == "tool":
            return {"role": "assistant",
                    "content": f"完成（工具返回 {messages[-1]['content']}）"}
        return tool_call_msg("training_list")
...
def make_env(responses):
    ...
    llm = responses if isinstance(responses, LoopLlm) else FakeLlm(responses)
    return registry, llm
...
async def test_graph_loops_multi_round_tool_calls():
    ...
    responses = [
        ...
        {"role": "assistant", "content": "查询完毕，共 3 页。"},
    ]
    registry, llm = make_env(responses)
    ...
    assert [m["role"] for m in final].count("tool") == 3  # 3 条 tool 回填
...
async def test_graph_recursion_limit_recovers():
    ...
    assert any(m["role"] == "tool" for m in final)
...
async def test_graph_concurrent_tasks_isolated():
    ...
    registry = ToolRegistry()
    registry.load([make_tool()])
    # 无状态 LLM 共享（真实场景 DeepSeekClient 亦无状态）：每个任务各跑一轮工具
    graph = build_graph(llm=StatelessLlm(), http=http, registry=registry, client=client)

    import asyncio
    msgs_a = [{"role": "system", "content": "s"}, {"role": "user", "content": "A问题"}]
    msgs_b = [{"role": "system", "content": "s"}, {"role": "user", "content": "B问题"}]
    ...
    # 各自收敛到结论，且都包含自己的工具回填
    assert fa[-1]["role"] == "assistant" and fa[-1]["content"].startswith("完成（")
    assert fb[-1]["role"] == "assistant" and fb[-1]["content"].startswith("完成（")
    # 状态未串扰：对方的消息不在自己的序列里
